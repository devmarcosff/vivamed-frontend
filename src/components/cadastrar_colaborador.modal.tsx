...
export default function CadastrarColaboradorModal({ openModal, closeModal }: any) {
  const { register, handleSubmit, formState: { errors }, reset } = useForm();
  const token = Cookie.get('accessToken');
  const [isRegistro, setIsRegistro] = useState<any>()

  const createCidadao = async (data: CadastroColaborador) => {
                      ...
                      <form className="w-full mt-10" onSubmit={handleSubmit(createCidadao)}>
                        ...
                        <div className="flex flex-wrap -mx-3 my-3">
                          <div className="w-full md:w-1/4 px-3 mb-6 md:mb-0">
                            ...
                            <input {...register('firstname', { required: 'Por favor preencha este campo' })} className={`${errors.firstname && 'border-red-500'} appearance-none shadow-md block w-full bg-gray-200 text-gray-700 border rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white`} id="grid-first-name" type="text" placeholder="29452021510025415" />
                            {
                              errors.firstname && <p className="text-red-500 text-xs italic">Por favor preencha este campo.</p>
                            }
                          </div>
                          <div className="w-full md:w-1/4 px-3 mb-6 md:mb-0">
                            <label className="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" htmlFor="grid-first-name">
                            ...
                            <input {...register('lastName', { required: 'Por favor preencha este campo' })} className={`${errors.lastName && 'border-red-500'} appearance-none shadow-md block w-full bg-gray-200 text-gray-700 border rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white`} id="grid-first-name" type="text" placeholder="Marcos Paulo Fernandes de Faria" />
                            {
                              errors.lastName && <p className="text-red-500 text-xs italic">Por favor preencha este campo.</p>
                            }
                          </div>
                          ...
                          <div className="w-full md:w-1/4 px-3">
                            ...
                            <input {...register('', { required: 'Por favor preencha este campo' })} className={`${errors.cpf && 'border-red-500'} appearance-none shadow-md block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500`} id="grid-last-name" type="text" placeholder="000.000.000.00" />
                            {
                              errors.cpf && <p className="text-red-500 text-xs italic">Por favor preencha este campo.</p>
                            }
                          </div>
